    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white shadow-sm text-gray-800">
      ...
      <div className="bg-gray-50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center py-4">
          ...
          <div className="hidden lg:flex flex-grow max-w-xl mx-auto items-center border bg-white border-gray-300 rounded-md ml-8">
            ...
            <button onClick={handleSearchClick} className="bg-pink-500 text-white p-3 rounded-r-md hover:bg-pink-600">
              <Search size={20} />
            </button>
          </div>
          ...
          <div className="flex items-center gap-4 md:gap-6 ml-auto">
            {session ? (
                <div className="hidden md:flex items-center gap-6 text-sm">
                    <a onClick={() => onNavigate('dashboard')} className="cursor-pointer hover:text-pink-500 flex items-center gap-2">
                        <User size={24} />
                        <span className="font-semibold">Dashboard</span>
                    </a>
                    <button onClick={onSignOut} className="cursor-pointer hover:text-pink-500 flex items-center gap-2">
                        <LogOut size={24} />
                        <span className="font-semibold">Sair</span>
                    </button>
                </div>
            ) : (
                <a onClick={() => onNavigate('login')} className="cursor-pointer hover:text-pink-500 flex flex-col items-center text-sm hidden md:flex">
                    <User size={24} />
                    <span className="font-semibold text-xs">Logar</span>
                </a>
            )}
            <a onClick={() => onNavigate('wishlist')} className="relative cursor-pointer hover:text-pink-500">
              <Heart size={24} />
              {wishlistItemCount > 0 && <span className="absolute -top-1 -right-2 text-xs bg-pink-500 text-white rounded-full w-4 h-4 flex items-center justify-center">{wishlistItemCount}</span>}
            </a>
            <a onClick={() => onNavigate('cart')} className="relative cursor-pointer hover:text-pink-500 flex items-center gap-2">
              <ShoppingCart size={24} />
              {cartItemCount > 0 && <span className="absolute -top-1 left-3 text-xs bg-pink-500 text-white rounded-full w-4 h-4 flex items-center justify-center">{cartItemCount}</span>}
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex items-center h-14">
          ...
          <div className="flex items-center gap-6 text-sm font-semibold ml-8">
            {mainNavLinks.map(link => (
              <a key={link.name} onClick={link.action} className="flex items-center gap-2 hover:text-pink-500 cursor-pointer">
      ...
      {isMobileMenuOpen && (
          <div className="lg:hidden fixed inset-0 bg-black/50 z-50" onClick={() => setIsMobileMenuOpen(false)}>
              <div className="fixed top-0 left-0 h-full w-4/5 max-w-sm bg-white shadow-lg p-6 overflow-y-auto" onClick={e => e.stopPropagation()}>
                  ...
                  {/* Mobile Search Bar */}
                  <div className="flex items-center border bg-gray-50 border-gray-300 rounded-md mb-6">
                    <input 
                      type="text" 
                      placeholder="Buscar produtos" 
                      value={searchTerm}
                      onChange={(e) => setSearchTerm(e.target.value)}
                      onKeyDown={handleKeyDown}
                      className="flex-grow px-4 py-2 rounded-l-md focus:outline-none bg-transparent" 
                    />
                    <button onClick={handleSearchClick} className="bg-pink-500 text-white p-3 rounded-r-md hover:bg-pink-600">
                      <Search size={20} />
                    </button>
                  ...
                  <nav className="flex flex-col gap-4">
                      <h3 className="font-semibold text-gray-500 text-sm uppercase">Departamentos</h3>
                      {allCategories.map(cat => (
                          <a key={cat.id} onClick={() => handleCategoryClick(cat.name)} className="cursor-pointer hover:text-pink-500 flex items-center gap-3">
                      ...
                      {mainNavLinks.map(link => (
                          <a key={link.name} onClick={() => { link.action(); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 cursor-pointer hover:text-pink-500">
                            {link.icon ? (
                      ...
                      {session ? (
                        <>
                          <a onClick={() => { onNavigate('dashboard'); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 cursor-pointer hover:text-pink-500">
                            <User size={20} />
                            <span>Dashboard</span>
                          </a>
                          <a onClick={() => { onSignOut(); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 cursor-pointer hover:text-pink-500">
                            <LogOut size={20} />
                            <span>Sair</span>
                          </a>
                        </>
                      ) : (
                        <a onClick={() => { onNavigate('login'); setIsMobileMenuOpen(false); }} className="flex items-center gap-3 cursor-pointer hover:text-pink-500">
                          <User size={20} />
                          <span>Logar</span>
                        </a>
                      )}

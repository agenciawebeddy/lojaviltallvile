...
import usePageHeader from '../hooks/usePageHeader'; // Importando hook para cabeçalho dinâmico
          ...
          return (
            <li key={cat.id} className="flex justify-between items-center">
              <a
                ...
                className={`cursor-pointer hover:text-gray-900 ${isSelected ? 'text-pink-600 font-semibold' : ''}`}
              >
                {cat.name}
              </a>
              <span className={`text-xs rounded-full w-6 h-6 flex items-center justify-center transition-colors ${isSelected ? 'bg-pink-600 text-white' : 'bg-gray-100 text-gray-500'}`}>
                {cat.product_count || 0}
              </span>
            </li>
          );
...
const ShopPage: React.FC<ShopPageProps> = ({ onAddToCart, allProducts, allCategories, initialCategory, onProductSelect, wishlistItems, onToggleWishlist, onNavigate, searchTerm = '', searchCategory = '' }) => {
  ...
  // Buscar dados do cabeçalho da página Shop do banco de dados
  const { headerData, isLoading: isLoadingHeader } = usePageHeader('shop');
  ...
  // Usar dados do banco de dados se disponíveis, caso contrário usar valores padrão
  const pageTitle = searchTerm
    ? `Resultados para "${searchTerm}"`
    : (headerData?.title || 'Nossa Loja');
  const pageDescription = searchTerm
    ? `Encontramos ${filteredAndSortedProducts.length} produtos.`
    : (headerData?.description || 'Explore nossa coleção completa de produtos selecionados.');
  const pageImageUrl = headerData?.image_url || 'https://picsum.photos/seed/shop/1920/300';
              ...
              <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <select
                    ...
                    className="bg-gray-50 border border-gray-300 rounded-md py-1 px-2 text-xs sm:py-2 sm:px-3 sm:text-sm text-gray-700 focus:ring-pink-500 focus:border-pink-500"
                  >
